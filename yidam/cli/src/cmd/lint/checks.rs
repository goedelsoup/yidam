...
///
/// A view rather than `&[Node]` for the reason [`crate::corpus::EdgeView`] is one: the derivation has two
...
/// declares properties every class inherits; it is not a class, [`crate::corpus::load_classes`] does not
...
mod tests {
    ...
    fn edge(relationship: &str, target: &str, direction: &str) -> crate::corpus::ClassEdge {
        crate::corpus::ClassEdge {
    ...
    fn loaded_class(name: &str, yaml: &str) -> (tempfile::TempDir, Vec<Class>) {
        ...
        let classes =
            crate::corpus::load_classes(dir.path(), &[p], &crate::corpus::Overlay::default());
        (dir, classes)
    }
    ...
    fn no_class_file_in_this_repository_carries_a_near_miss() {
        ...
        let classes =
            crate::corpus::load_classes(&root, &paths, &crate::corpus::Overlay::default());
    ...
    fn the_loader_keeps_an_empty_used_by_apart_from_an_absent_one() {
        ...
        let mut overlay = crate::corpus::Overlay::default();
        overlay.set(
            declared.clone(),
            "---\nname: a\nused-by: []\n---\n\n# A\n".into(),
        );
        ...
        let sources = crate::corpus::load_sources(root, &[declared, absent], &overlay);
