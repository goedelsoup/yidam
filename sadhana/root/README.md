...
Fields: corpus node count, open questions, catalog entries, index freshness,
        genesis date. Phase counts are not here: this block is committed and gated,
        so it holds only what every checkout of the commit agrees on. Ask
        `yidam phases`, or `yidam status --format json`, for those.
-->
