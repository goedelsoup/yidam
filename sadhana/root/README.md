...
Fields: corpus node count, open questions, catalog entries, claim counts, genesis date.
        Phase counts and index freshness are not here: this block is committed and gated,
        so it holds only what every checkout of the commit agrees on — and a phase ref and
        a built index are both facts about one machine. Ask `yidam phases`,
        `yidam index-status`, or `yidam status --format json`, for those.
-->
...
Regenerated by: `yidam regen`, which runs `yidam vault-status` with every other generator.
...
the same in every clone. `yidam vault status` answers what is on *this* machine, and is
read-only; the hyphenated command is this generator and rewrites this file.
-->
